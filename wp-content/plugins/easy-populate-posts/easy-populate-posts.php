 ...
 * @package spp
 *
 * Copyright (C) 2015-2021 Iulia Cazan
 *
...
define( 'SPP_PLUGIN_VERSION', 3.71 );
...
class SISANU_Popupate_Posts {
	...
	public static function spp_save_settings( $return = true ) { // phpcs:ignore
		if ( self::spp_current_user_can() && self::spp_validate_nonce() ) {
			...
			foreach ( $ds as $key => $value ) {
				if ( isset( $pspp[ $key ] ) ) {
					if ( 'start_counter' === $key ) {
						...
						if ( 'gutenberg_block' === $key ) {
							$ds_new[ $key ] = (int) ! empty( $ds_new[ $key ] );

							if ( 0 === $ds_new['gutenberg_block'] ) {
								$ds_new['gutenberg_drop_cap'] = 0;
							}
						}

						if ( 'gutenberg_drop_cap' === $key ) {
							$ds_new[ $key ] = (int) ! empty( $ds_new[ $key ] );
						}
					}
				}
			...
			if ( ! isset( $pspp['gutenberg_block'] ) ) {
				$ds_new['gutenberg_block']    = 0;
				$ds_new['gutenberg_drop_cap'] = 0;
			}

			if ( ! isset( $pspp['gutenberg_drop_cap'] ) ) {
				$ds_new['gutenberg_drop_cap'] = 0;
			}
			...
			<form id="spp_settings_frm" action="" method="post" class="spp">
				...
				<div class="options-boxes">
					<div>
						...
						<h4><?php esc_html_e( 'Title Prefix', 'spp' ); ?></h4>
						...
						<p id="spp_title_prefix_counter" class="hidden">
							<input type="number" class="f-right" name="spp[start_counter]" id="spp_start_counter" value="<?php echo esc_attr( self::$settings['start_counter'] ); ?>" size="20" disabled="disabled">
							<em><?php esc_html_e( 'Start the auto-increment prefix number from this', 'spp' ); ?></em>
						</p>
					...
					<div>
						...
						<div id="spp_specific_date_wrap"
							...
							<p>
								<input type="time" name="spp[specific_hour]" id="spp_specific_hour" value="<?php echo esc_attr( self::$settings['specific_hour'] ); ?>" size="15" placeholder="<?php echo esc_attr( gmdate( 'H:i' ) ); ?>">
					...
					<div>
						<h3><?php esc_html_e( 'Terms', 'spp' ); ?></h3>
						...
						<h4><?php esc_html_e( 'Taxonomy', 'spp' ); ?> 1</h4>
						...
						<p>
							<em><?php esc_html_e( 'separate terms names by comma, these will be created if not found', 'spp' ); ?></em>
							<br>
							<input type="text" name="spp[term_slug]" id="spp_term_slug" value="<?php echo esc_attr( str_replace( ',', ', ', self::$settings['term_slug'] ) ); ?>" size="10" placeholder="<?php esc_attr_e( 'name', 'spp' ); ?>">
						</p>
						...
						<h4><?php esc_html_e( 'Taxonomy', 'spp' ); ?> 2</h4>
						...
						<p>
							<em><?php esc_html_e( 'separate terms names by comma, these will be created if not found', 'spp' ); ?></em>
							<br>
							<input type="text" name="spp[term_slug2]" id="spp_term_slug2" value="<?php echo esc_attr( str_replace( ',', ', ', self::$settings['term_slug2'] ) ); ?>" size="10" placeholder="<?php esc_attr_e( 'name', 'spp' ); ?>">
						</p>
					...
					<div>
						...
						for ( $k = 1; $k <= 5; ++ $k ) {
							...
							<input type="text" class="first"
								...
								value="<?php echo esc_attr( self::$settings[ 'meta_key' . $mk ] ); ?>"
								size="15" placeholder="<?php esc_attr_e( 'key', 'spp' ); ?>">
							<input type="text" class="second"
								name="spp[meta_value<?php echo esc_attr( $mk ); ?>]"
								id="spp_meta_value<?php echo esc_attr( $mk ); ?>"
								value="<?php echo esc_attr( maybe_serialize( self::$settings[ 'meta_value' . $mk ] ) ); ?>" size="10" placeholder="<?php esc_attr_e( 'value', 'spp' ); ?>">
							<?php
						}
						?>
					...
					<div>
						...
						<p>
							<em>
								<a onclick="sppCopyImagesList();" class="button"><?php esc_html_e( 'use the plugin sample images', 'spp' ); ?></a>
								<br>
		...
		for ( $i = 0; $i < self::$settings['max_number']; $i ++ ) {
			...
			if ( ! empty( $name ) ) {
				...
				if ( ! empty( $post_id ) ) {
					...
					$image_embed    = ( ! empty( $photo_src ) ) ? '<img src="' . esc_url( $photo_src ) . '" width="80" style="float: left; margin-right: 10px;" loading="lazy">' : '';
					$return_result .= '
					<li>
						' . $image_embed . '' . __( 'Status', 'spp' ) . ' <em class="tag-preview">' . $status . '</em>,<br>' . __( 'Date', 'spp' ) . ' <em class="tag-preview">' . $date . '</em><br><br><a href="' . admin_url( 'post.php?post=' . $post_id . '&action=edit' ) . '" class="button" style="float:right">' . __( 'Edit post', 'spp' ) . '</a><div class="clear"></div><hr><h2>' . $name . '</h2>';
					if ( count( $tags ) !== 0 ) {
